...
class AdminNotice {
    ...
    public function __construct() {
        ...
        if ( get_option( $this->dismiss_notice_key ) !== 'yes' ) {
			add_action( 'admin_notices', [ $this, 'newsium_admin_notice' ], 0 );
			add_action( 'wp_ajax_aft_notice_dismiss', [ $this, 'newsium_notice_dismiss' ] );
		}
	
    }

    function newsium_admin_notice(){
        ...
        $this->newsium_dashboard_notice_content();
        echo '</div>';

       
    }

    function newsium_dashboard_notice_content(){
		if ( file_exists( WP_PLUGIN_DIR . '/af-companion/af-companion.php' ) ) {
			if(is_plugin_active('af-companion/af-companion.php')){
				$af_companion_title = __( 'Get Starter Sites', 'newsium' );
				$af_companion_url = site_url( ).'/wp-admin/admin.php?page=af-companion';
			}else{
            $af_companion_title = __( 'Activate AF Companion', 'newsium' );
            ...
            $af_companion_title = __( 'Install AF Companion', 'newsium' );
        ...
        $notice_header = sprintf(
			...
			esc_html__( 'Howdy!', 'newsium' ),
			sprintf(
				esc_html__( '%s is now installed and ready to use. We\'ve assembled some links to get you started.', 'newsium' ),
				$this->theme_name
			)
		);
		...
		$demo_link = "https://afthemes.com/products/newsium/";
		$notice_starter_msg =sprintf(
			...
			__( 'General info', 'newsium' ),
			esc_html__( 'All of the features provided by the theme are now ready to use; Here, we have gathered all of the essential details and helpful links for you and your better experience with the theme. Once again, Thanks for using our theme!', 'newsium' ),
			$af_companion_url,
			$af_companion_title,
			esc_url($demo_link),
			esc_html__('Demos/product','newsium'),
			esc_url( admin_url() ."themes.php?page=".$this->page_slug ),
			esc_html__('Theme dashboard','newsium')
		...
		$notice_external_msg =sprintf(
			...
			<div class="aft-documentation-links">
				<div>
					<a href="https://docs.afthemes.com/newsium/" target="_blank"><span aria-hidden="true" class="dashicons dashicons-external"></span>%3$s</a>
					<a href="https://www.youtube.com/watch?v=4NCnqvf9vz8&list=PL8nUD79gscmiPRjrxRab1VQCmbbgXIG9X" target="_blank"><span aria-hidden="true" class="dashicons dashicons-external"></span>%4$s</a>
			...
			__( 'Documentation', 'newsium' ),
			esc_html__( 'Please check our full documentation for detailed information on how to setup and customize the theme.', 'newsium' ),
			esc_html__( 'Docs', 'newsium' ),
			esc_html__( 'Videos', 'newsium' ),
			esc_html__( 'Blog', 'newsium' ),
			esc_html__( 'Return to your dashboard', 'newsium' )
    ...
    public function newsium_notice_dismiss() {
